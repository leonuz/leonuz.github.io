---
layout: default
title: whoami
---

<div class="post">
	<h1 class="pageTitle">About Me</h1>

	<p>I'm <strong>Leo</strong> — IT Security Engineer with over two decades in cybersecurity. Electrical Engineer with a Master's degree in Telecommunications from the <a href="https://www.ula.ve/" target="_blank">Universidad de los Andes</a> (Mérida, Venezuela). Based in the United States since December 2021.</p>

	<p>My approach to security has always been dual: understand how attackers think, build better defenses. I've spent my career doing both — from training security professionals across Latin America to hunting threats and hardening systems in the field. These days I'm particularly focused on the intersection of <strong>AI and defensive security</strong>, and I'm in the final stages of building <a href="https://github.com/leonuz/pfchat" target="_blank">PfChat</a>, an <a href="https://github.com/openclaw" target="_blank">OpenClaw</a> skill that brings conversational AI to network security operations.</p>

	<p>From 2001 to 2013, I was part of the <a href="https://www.internetsociety.org/news/press-releases/2008/leading-latin-american-foundation-eslared-receives-10th-annual-jonathan-b-postel-service-award-promoting-internet-latin-america/" target="_blank">ESLARED</a> instructor team — a foundation recognized by the Internet Society with the <em>Jonathan B. Postel Service Award</em> for promoting internet development in Latin America. During those 12 years I trained professionals in Cybersecurity, Wireless Networks, and VoIP across Latin America and the Caribbean. Teaching shaped how I think about security: if you can't explain it clearly, you don't understand it well enough.</p>

	<p>I also play CTFs to keep my offensive instincts sharp. Since 2021 I've participated in over 100 CTFs across platforms worldwide, publishing writeups for 13 of them — they live on this blog. You can also track me on <a href="https://tryhackme.com/p/leonuz" target="_blank">TryHackMe</a> and on <a href="https://ctftime.org/team/156755" target="_blank">CTFtime</a>.</p>
	<p>Three of my writeups have been officially recognized by their organizers:</p>
	<ul>
		<li><strong>2nd Best Write-up</strong> — Deloitte Hacky Holidays 2021</li>
		<li><strong>Top-10 Best Write-up</strong> — Deloitte Hacky Holidays 2022</li>
		<li><strong>Top-3 Best Write-up</strong> — Space Heroes CTF 2023 (FITSEC / Florida Tech)</li>
	</ul>
	<p>Some notable rankings among those competitions:</p>
	<ul>
		<li><strong>2nd / 114 teams</strong> — OWASP Panama CTF 2022</li>
		<li><strong>2nd / 48 teams</strong> — WorlParty CTF 2022 (Spain)</li>
		<li><strong>3rd / 45 teams</strong> — Kavacon CTF 2022 (Paraguay)</li>
		<li><strong>4th / 385 teams</strong> — DragonJarCon CTF 2021 (Colombia)</li>
	</ul>

	<h2>Certifications</h2>
	<ul>
		<li>(ISC)² CC — Certified in Cybersecurity</li>
		<li>CompTIA Security+</li>
		<li>eCPPT — eLearnSecurity Certified Professional Penetration Tester</li>
		<li>eJPT — eLearnSecurity Junior Penetration Tester</li>
		<li>ITIL v4 Foundation</li>
		<li>(ISC)² CISSP — Candidate (exam scheduled October 2026)</li>
	</ul>

	<h2>Technologies &amp; Platforms</h2>

	<h3>SIEM &amp; Threat Detection</h3>
	<ul>
		<li>Microsoft Sentinel</li>
		<li>Rapid7</li>
	</ul>

	<h3>Endpoint Protection</h3>
	<ul>
		<li>Microsoft Defender Suite</li>
		<li>CrowdStrike</li>
	</ul>

	<h3>Email Security</h3>
	<ul>
		<li>Proofpoint</li>
		<li>Microsoft Exchange</li>
		<li>PowerDMARC</li>
		<li>EasyDMARC</li>
		<li>DMARC / SPF</li>
	</ul>

	<h3>Network Security</h3>
	<ul>
		<li>Cisco Umbrella SIG</li>
		<li>Cisco Meraki</li>
		<li>Zero Trust Network Access (ZTNA)</li>
	</ul>

	<h3>Identity &amp; Cloud</h3>
	<ul>
		<li>Microsoft Entra</li>
		<li>Microsoft Azure</li>
	</ul>

	<h3>ITSM</h3>
	<ul>
		<li>ServiceNow</li>
	</ul>

	<h2>Technical Skills</h2>
	<ul>
		<li>Red Team / Blue Team Operations</li>
		<li>NOC / SOC</li>
		<li>ISO/IEC 27001 — Information Security Management System (ISMS)</li>
		<li>Network Administration &amp; System Administration</li>
		<li>AI-focused Security Solutions</li>
	</ul>

	<h2>Community &amp; Memberships</h2>
	<ul>
		<li>IEEE Member since 1999</li>
		<li>Linux Registered User since 2004</li>
		<li>ISOC Member since 2009</li>
		<li><a href="https://tryhackme.com/p/leonuz" target="_blank">TryHackMe</a> — active CTF player</li>
		<li><a href="https://github.com/leonuz/CTFs" target="_blank">CTF Writeups</a> on GitHub</li>
	</ul>

	<h2>A Bit More</h2>
	<ul>
		<li>Research and writing instincts</li>
		<li>Teacher at heart</li>
		<li>Passion for learning</li>
		<li>Founder of <a href="https://www.knowdefend.com" target="_blank">Defense Security LLC</a> — independent cybersecurity consulting</li>
		<li>Proud husband and father of two</li>
	</ul>

	<h2>Badges &amp; Credentials</h2>
	<table class="center">
		<tr>
			<th>Platform</th>
			<th>Profile</th>
		</tr>
		<tr>
			<td style="text-align:center">Acredible</td>
			<td style="text-align:center"><a href="https://www.credential.net/profile/leonardouzcategui/wallet" target="_blank"><img src="/assets/img/acredible.png" alt="Acredible"></a></td>
		</tr>
		<tr>
			<td style="text-align:center">Credly</td>
			<td style="text-align:center"><a href="https://www.credly.com/users/leonuz/badges" target="_blank"><img src="/assets/img/credly.png" alt="Credly"></a></td>
		</tr>
		<tr>
			<td style="text-align:center">Badgr</td>
			<td style="text-align:center"><a href="https://api.badgr.io/public/assertions/6eQ4G5z3Tii6u3vIwBakTw?identity__email=uzcategui%40gmail.com" target="_blank"><img src="/assets/img/badgr.png" alt="Badgr"></a></td>
		</tr>
	</table>

	<figure>
		<a href="https://en.wikipedia.org/wiki/Jon_Hall_(programmer)" target="_blank"><img src="/assets/img/maddog.png" alt="Jon Maddog Hall"></a>
		<figcaption>With Jon "Maddog" Hall at a Linux conference in Coro, Falcón, Venezuela — November 2007.</figcaption>
	</figure>
</div>
