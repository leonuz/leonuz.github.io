...
Everything indicates that the **AI** has encrypted the message using [RSA (Rivest, Shamir, & Adleman (public key encryption technology))](https://en.wikipedia.org/wiki/RSA_(cryptosystem))  
...
After all it is a CTF, it is important to solve the challenges by knowing what it is all about, interacting with the different components and most importantly, **Learning**.
